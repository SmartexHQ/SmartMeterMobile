...
String? reconTagValidator(String? value) {
  final usernameRegExp = RegExp(r'^[A-Za-z0-9]{5,30}$');

  if (value == '' || value == null) {
    return 'Recon tag must not be empty!';
  } else if (value.trim().length <= 4) {
    return 'Recon tag must be min 5 characters long!';
  } else if (usernameRegExp.hasMatch(value) != true) {
    return 'Recon tag must be at least 5 characters, and can only contain letters, numbers, and underscores.';
  }
  return null;
}
